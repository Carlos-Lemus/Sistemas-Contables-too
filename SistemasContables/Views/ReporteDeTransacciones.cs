...
{
    public partial class ReporteDeTransacciones : Form
    {

        private ReportesController reporteControler;
        ...
        public ReporteDeTransacciones(LibroDiario libroDiario)
        {
            InitializeComponent();

            lblPeriodo.Text = libroDiario.Periodo;
            dpMes.Value = DateTime.Now;
            dpMes.CustomFormat = "MM/yyyy";

            reporteControler = new ReportesController();
        ...
        private void llenarReporte(int idLibroDiario)
        {
            ...
            listaCodigos = reporteControler.getListCodigos(idLibroDiario);
            ...
            foreach (string codigo in listaCodigos)
            {
                ...
                partida = reporteControler.getPartida(nivel3);

                if (partida != null)
                {
                    listaPartidas = reporteControler.getListPartida(nivel3, idLibroDiario);
        ...
        public void llenarRegistros(double totalMontos)
        {
            foreach (Partida partidaAux in listaPartidas)
            {
                ...
                {
                    tableReporteDeTransacciones.Rows.Add(partidaAux.Fecha, partidaAux.Detalle, redondear(partidaAux.ListaCuentasPartida[0].Haber));
                }
            }
        }
